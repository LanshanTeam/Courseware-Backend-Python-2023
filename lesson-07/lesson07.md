...
> 可以简单理解为把单个输入变成多个头，每个头进行自注意力机制，最后用$W^o$矩阵变换得到一组输出。
...
1. 将“早上好”通过Encoder编码，并将最后 $t=3$ 时刻的隐藏层状态 ℎ3 作为语义向量。
...
>  $\overline{h_t}$ 代表Encoder状态， $h_t$ 代表Decoder状态，$\widetilde{h_t}$ 代表Attention Layer输出的最终Decoder状态
